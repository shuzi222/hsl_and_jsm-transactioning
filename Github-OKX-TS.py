    ...
    try:
        ...
        for symbol in symbols:
            ...
            for key in ['RSI_BUY_VALUE', 'RSI_SELL_VALUE', 'BUY_RATIO', 'LEVERAGE', 'TAKE_PROFIT', 'STOP_LOSS',
                        'ATR_BASE', 'BUY_RATIO_MIN', 'BUY_RATIO_MAX', 'ATR_TP_MULTIPLIER', 'ATR_SL_MULTIPLIER']:  # MODIFIED: Added new parameters
...
def place_order(symbol, side, pos_side, quantity, atr=None):
    ...
    try:
        ...
        # MODIFIED: Use ATR for TP and SL if available, fallback to percentage-based
        if atr is None:
            ...
            tp_price = round(current_price + atr * params['ATR_TP_MULTIPLIER'] if pos_side == 'long' else current_price - atr * params['ATR_TP_MULTIPLIER'], -int(np.log10(tick_sz)))
            sl_price = round(current_price - atr * params['ATR_SL_MULTIPLIER'] if pos_side == 'long' else current_price + atr * params['ATR_SL_MULTIPLIER'], -int(np.log10(tick_sz)))
            logging.info(f"{symbol} ATR 计算: 当前价格={current_price:.2f}, ATR={atr:.2f}, 止盈倍数={params['ATR_TP_MULTIPLIER']}, 止损倍数={params['ATR_SL_MULTIPLIER']}, 止盈价格={tp_price:.2f}, 止损价格={sl_price:.2f}")
...
def check_take_profit_stop_loss(symbol, long_qty, short_qty, long_avg_price, short_avg_price, current_price, rsi, macd, signal, atr=None):  # MODIFIED: Added atr parameter
    ...
    try:
        ...
        if long_qty > 0 and long_avg_price > 0:
            ...
            # MODIFIED: Check ATR-based TP and SL
...
def execute_trading_logic(symbol):
    ...
    try:
        ...
        # 检查止盈止损或反向信号
        pos_side, qty, reason = check_take_profit_stop_loss(symbol, long_qty, short_qty, long_avg_price, short_avg_price, state[symbol]['current_price'], state[symbol]['latest_rsi'], macd, signal, atr=atr)  # MODIFIED: Pass atr
        if pos_side and qty > 0:
            order = place_order(symbol, 'sell' if pos_side == 'long' else 'buy', pos_side, qty, atr=atr)  # MODIFIED: Pass atr
            if order:
        ...
        if long_qty == 0 and short_qty == 0:
            ...
            if state[symbol]['latest_rsi'] <= params['RSI_BUY_VALUE']:
                ...
                if quantity >= min_quantity:
                    order = place_order(symbol, 'buy', 'long', quantity, atr=atr)  # MODIFIED: Pass atr
                    if order:
                ...
                quantity = min((usdt_balance * dynamic_buy_ratio) / state[symbol]['current_price'] * params['LEVERAGE'], max_quantity)
                if quantity >= min_quantity:
                    order = place_order(symbol, 'sell', 'short', quantity, atr=atr)  # MODIFIED: Pass atr
                    if order:
